classdef State
    methods (Static)
        function Tout = compose(X1, X2)
            ...
            [Rb1, pb1, vb1, Rlf1, plf1, Rrf1, prf1, ba1, bg1] = Estimation.DLGEKF_RIE.State.extract(X1);
            [Rb2, pb2, vb2, Rlf2, plf2, Rrf2, prf2, ba2, bg2] = Estimation.DLGEKF_RIE.State.extract(X2);
            ...
            estimate_bias = Estimation.DLGEKF_RIE.State.is_estimate_bias_enabled(X1);
            Tout = Estimation.DLGEKF_RIE.State.construct(Rbout, pbout, vbout, ...
                Rlfout, plfout, ...
                Rrfout, prfout, ...
        ...
        function Tout = inverse(X)
            [Rb, pb, vb, Rlf, plf, Rrf, prf, ba, bg] = Estimation.DLGEKF_RIE.State.extract(X);
            ...
            estimate_bias = Estimation.DLGEKF_RIE.State.is_estimate_bias_enabled(X);
            Tout = Estimation.DLGEKF_RIE.State.construct(Rbout, pbout, vbout, ...
                Rlfout, plfout, ...
                Rrfout, prfout, ...
                baout, bgout, estimate_bias);
        ...
        function v = vee(g)
            [gbase, glf, grf, gbiases] = Estimation.DLGEKF_RIE.State.extractLieAlgebra(g);
            vbase = LieGroups.SE_2_3.vee(gbase);
            vlf = LieGroups.SE3.vee(glf);
            vrf = LieGroups.SE3.vee(grf);
            
            v = [vbase; vlf; vrf];
            if Estimation.DLGEKF_RIE.State.is_estimate_bias_enabled(g)
        ...
        function g = hat(v)
            estimate_bias = Estimation.DLGEKF_RIE.State.is_estimate_bias_enabled(v);
            [vbase, vlf, vrf, vbias] = Estimation.DLGEKF_RIE.State.splitVector(v, estimate_bias);
            ...
            g = Estimation.DLGEKF_RIE.State.constructLieAlgebra(gbase, glf, grf, gbiases, estimate_bias);
        end
        
        function T = exp(g)
            v = Estimation.DLGEKF_RIE.State.vee(g);
            T = Estimation.DLGEKF_RIE.State.exphat(v);
        end
        ...
        function T = exphat(v)
            ...
            T = Estimation.DLGEKF_RIE.State.construct(Rb, pb, vb, ...
                Rlf, plf, ...
                Rrf, prf, ...
                ba, bg, estimate_bias);
        end
        ...
        function g = log(X)
            v = Estimation.DLGEKF_RIE.State.logvee(X);
            g = Estimation.DLGEKF_RIE.State.hat(v);
        end
        
        function v = logvee(X)
            estimate_bias = Estimation.DLGEKF_RIE.State.is_estimate_bias_enabled(X);
            [Xbase, Xlf, Xrf, Xbias] = Estimation.DLGEKF_RIE.State.extractByParts(X);
        ...
        function [Rb, pb, vb, Rlf, plf, Rrf, prf, ba, bg]  = extract(X)
            estimate_bias = Estimation.DLGEKF_RIE.State.is_estimate_bias_enabled(X);
            [baseSE23, lfootSE3, rfootSE3, biasesTn] = Estimation.DLGEKF_RIE.State.extractByParts(X);
        ...
        function [gbase, glf, grf, gbiases] = extractLieAlgebra(g)
            estimate_bias = Estimation.DLGEKF_RIE.State.is_estimate_bias_enabled(g);
            gbase = g(1:5, 1:5);
            glf = g(6:9, 6:9);
            grf = g(10:13, 10:13);
