function [rotError, posError, velError, ATErot, ATEpos, ATEvel, RPErot, RPEpos] = getLeftInvariantErrorMetrics(pos, rpy, vel, ...
...
for idx = 1:n
    ...
    % left invariant error ate
    deltaR = R_i_hat'*R_i;
    deltaPhi = LieGroups.SO3.logvee(deltaR);
    deltap = R_i_hat'*(p_i - p_i_hat);
    deltav = R_i_hat'*(v_i - v_i_hat);
    ...
    if (iplusdelta) < n
        ...
        E_i = LieGroups.SE3.compose(RPhatinv, RP);
