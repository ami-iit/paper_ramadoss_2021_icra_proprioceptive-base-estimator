classdef Filter < handle
    ...
    methods
        ...
        function [X, P, BaseLinkState, DebugOut, obj] = advance(obj, t, omega, alpha, encoders, contacts)
            ...
            [R, p, v, ~, ~, ~, ~, ~, bg] = Estimation.DLGEKF.State.extract(obj.X);
            q = Utils.rot2quat(R);
    ...
    methods (Access = private)
        function obj = predictState(obj, omega, alpha, contact, dt)
            [R, ~, v, ~, ~, ~, ~, ba, bg] = Estimation.DLGEKF.State.extract(obj.X);
            ...
            %%--propagate the mean
            %%%%%%%%%%%%%%%%%%%%%%%
            Omega_lifted = Estimation.DLGEKF.State.exphat(Omega);   
            obj.X = Estimation.DLGEKF.State.compose(obj.X, Omega_lifted);
                       
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%-- Left parametrized velocity Jacobian slantFk
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            accLinCross = Utils.skew(R'*v*dt + 0.5*R'*obj.g*dt*dt); %default
            gCross = Utils.skew(R'*obj.g*dt); %default
                                    
            slantFkbase = [zeros(3)    accLinCross     eye(3)*dt; ...
            ...
            if obj.options.enable_bias_estimation
                slantFkbasebias = [-0.5*eye(3)*dt*dt     zeros(3); ...
                                            zeros(3)   -eye(3)*dt; ...
                                          -eye(3)*dt     zeros(3); ...
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            OmegaLiftedInverse = Estimation.DLGEKF.State.exphat(-Omega);

            AdjOmegaLiftedInverse = Estimation.DLGEKF.State.AdjointMatrix(OmegaLiftedInverse);
            rightJacobianOmega = Estimation.DLGEKF.State.rightJacobian(Omega);
            Fk = AdjOmegaLiftedInverse + (rightJacobianOmega*slantFk);
            ...
            obj.P = Fk*obj.P*Fk'+ rightJacobianOmega*Qk*rightJacobianOmega';
            obj.P = (obj.P + obj.P')./2;
        ...
        function obj = updateState(obj, deltaY, H, R)
            ...
            deltaX_lifted = Estimation.DLGEKF.State.exphat(deltaX);

            obj.X = Estimation.DLGEKF.State.compose(obj.X, deltaX_lifted);
            
            I = eye(size(obj.P));
            rightJacobiandeltaX = Estimation.DLGEKF.State.rightJacobian(deltaX);
        ...
        function obj = updateKinematics(obj, encoders, contacts, dt)
            [A_R_IMU, p, ~, A_R_LF, pl, A_R_RF, pr, ~, ~] = Estimation.DLGEKF.State.extract(obj.X);
